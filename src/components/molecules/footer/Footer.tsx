...
const Footer = () => {
  ...
  useEffect(() => {
    if (size(entities)) {
      setData(get(head(entities), FOOTER));
    }
  }, [entities]);
        ...
        <div>
          <footer className="footer border-t lg:border-none px-4 lg:px-10 pt-10 mt-10 lg:pt-20 pb-10 lg:flex">
            <div className="flex flex-col items-center lg:block w-full lg:w-80">
              ...
              <Button isRtl={false} variant="btn-primary" link={get(data, "header.buttonLink")}>
                <Image
                  loading={"lazy"}
              ...
              <SocialMenu links={get(data, "footer.footer_links.data")} className="gap-x-2 mt-6" />
            </div>
            <div className="hidden lg:flex lg:flex-1 gap-x-48 lg:pl-32">
              <FooterMenu className="gap-y-3 flex flex-col" items={get(data, "body")} />
            </div>
          </footer>
